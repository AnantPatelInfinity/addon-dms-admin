...
import { getDealerCart, inDealerCartQty, outDealerCartQty, removeDealerCart } from '../../../middleware/cart/cart';
import { inCartQtyReset, outCartQtyReset, removeCartReset } from '../../../slices/cart.slice';
...
const DealerCart = () => {
            ...
            <div className="container mt-4 text-center py-5">
                <div className="card shadow-sm">
                    <div className="card-body py-5">
                        ...
                        <p className="text-muted mb-4">Add some products to see them here</p>
                        <button className="btn btn-primary" type='button' onClick={() => navigate(DEALER_URLS.PRODUCT_LIST)}>
                            Continue Shopping
                        </button>
                    </div>
                </div>
    ...
    const handleIncreaseQty = (productId, currentQty) => {
        ...
        dispatch(inDealerCartQty(formData));
    };
    ...
    const handleDecreaseQty = (productId, currentQty) => {
        ...
        formData.append("productId", productId);
        formData.append("quantity", newQty);
        dispatch(outDealerCartQty(formData));
    };
    ...
    const handleRemoveItem = (productId) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                dispatch(removeDealerCart(formData));
            }
        });
    };
    ...
    return (
        <div className="container mt-3 mt-md-4 mb-5">
            <h2 className="mb-3 mb-md-4 text-center">Your Cart - {cartData?.dealerName}</h2>
                                ...
                                <div key={product.productId}>
                                    <div className="row align-items-center g-2 py-2 py-md-3">
                                        ...
                                        <div className="col-4 col-md-2">
                                            <img
                                                src={product?.productDetails?.images[0]?.url || 'https://via.placeholder.com/100'}
                                        ...
                                        <div className="col-8 col-md-4">
                                            <h6 className="mb-1 mb-md-2">{product.productDetails.name}</h6>
                                            <p className="small text-muted mb-1 d-none d-md-block">
                                                {product?.productDetails?.companyName} • {product.productDetails.categoryName}
                                            </p>
                                        ...
                                        <div className="col-12 col-md-3 mt-2 mt-md-0">
                                            <div className="d-flex align-items-center justify-content-start justify-content-md-center">
                                                <button
                                                    ...
                                                    onClick={() => handleDecreaseQty(product.productId, product.quantity)}
                                                    disabled={product?.quantity <= 1 || (outCartQtyLoading && updatingProductId === product.productId)}
                                                >
                                                    {(outCartQtyLoading && updatingProductId === product.productId) ? (
                                                ...
                                                <button
                                                    className="btn btn-outline-secondary btn-sm py-1 px-2"
                                                    onClick={() => handleIncreaseQty(product.productId, product.quantity)}
                                                    disabled={product?.quantity >= 10 || (inCartQtyLoading && updatingProductId === product.productId)}
                                                >
                                                    {(inCartQtyLoading && updatingProductId === product.productId) ? (
                                                        <span className="spinner-border spinner-border-sm" role="status" aria-hidden="true" />
                                                    ) : (
                                                        <Plus size={14} />
                                                    )}
                                        ...
                                        <div className="col-6 col-md-2 text-start text-md-end">
                                            <h6 className="mb-0">₹{(product?.price * product?.quantity).toLocaleString()}</h6>
                                            <p className="small text-muted mb-0">
                                                (Includes GST: ₹{(calculateGstAmount(product?.price, product?.productDetails?.gst) * product?.quantity).toLocaleString()})
                                            </p>
                                        </div>
                                        ...
                                        <div className="col-6 col-md-1 text-end">
                                            <button
                                                ...
                                                disabled={removeCartLoading && updatingProductId === product.productId}
                                            >
                                                {(removeCartLoading && updatingProductId === product.productId) ? (
                                                    <span className="spinner-border spinner-border-sm" role="status" aria-hidden="true" />
                                                ) : (
                                                    <Trash2 size={14} />
                                                )}
                    ...
                    <div className="card shadow-sm sticky-md-top">
                        <div className="card-body">
                            ...
                            <div className="d-flex justify-content-between mb-2">
                                ...
                                <span>
                                    ₹{cartData.products.reduce((total, product) => {
                            ...
                            <button className="btn btn-primary w-100 py-2 mb-2" onClick={() => navigate(DEALER_URLS.CHECKOUT)}>
                                Proceed to Checkout
                            </button>
                            <button className="btn btn-outline-secondary w-100 py-2" type='button' onClick={() => navigate(DEALER_URLS.PRODUCT_LIST)}>
                                Continue Shopping
                            </button>
                        </div>
                    </div>
