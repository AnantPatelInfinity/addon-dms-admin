...
const CustomerOtpVerify = () => {
    ...
    useEffect(() => {
        const storedExpiry = localStorage.getItem('cu_otpExpiry');
        const now = Date.now();
        ...
        if (storedExpiry) {
            ...
            localStorage.setItem('cu_otpExpiry', expiry.toString());
            setTimeLeft(120); 
            setResendEnabled(false);
        }
    }, []);
    ...
    const handleVerify = async (e) => {
        ...
        setLoading(true);
        try {
            const res = await post(`${CUSTOMER_URLS.OTP_VERIFY}`, { email, otp });
            localStorage.removeItem('cu_otpExpiry');
            const { success, message } = res;
            if (success) {
                toast.success(message);
                navigate(CUSTOMER_URLS.RESET_PASSWORD, { state: email });
            } else {
                toast.error(message);
            }
        } catch (err) {
    ...
    const handleResendOtp = async () => {
        try {
            ...
            const res = await post(`${CUSTOMER_URLS.FORGET_PASS}`, { email });
            const { success, message } = res;
            if (success) {
                toast.success(message);
                setResendEnabled(false);
                const newExpiry = Date.now() + 120000; 
                localStorage.setItem('cu_otpExpiry', newExpiry.toString());
                setTimeLeft(120); 
            } else {
                toast.error(message);
            }
