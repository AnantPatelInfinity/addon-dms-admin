...
const CompanyList = () => {
  ...
  const getCompanyData = async () => {
    try {
      ...
      const response = await get(
        `/admin/get-company?firmId=${adminStorage.DX_AD_FIRM}`
      );
  ...
  const handleDelete = async (elem) => {
    ...
    if (confirmResult.isConfirmed) {
      try {
        const response = await deleteById(`/admin/delete-company`, elem._id);
        if (response.success) {
  ...
  const handleStatusUpdate = async (elem, status) => {
    ...
    const confirmResult = await Swal.fire({
      ...
      text: `Are you sure you want to set ${elem?.name
    ...
    if (confirmResult.isConfirmed) {
      try {
        const response = await post(
          `/admin/update-company-status/${elem?._id}?status=${status}`
        );
        if (response.success) {
          Swal.fire(
            "Updated!",
            `${elem?.name} status updated to ${actionLabel}.`,
            "success"
          );
          setLoading(true); // refresh list
        } else {
                        ...
                        {visibleColumns.name && (
                          <td>
                            <h2 className="d-flex align-items-center">
                              ...
                              <a className="d-flex flex-column">{elem?.name}</a>
                            </h2>
                          </td>
                        )}
                        {visibleColumns.email && <td>{elem.email}</td>}
                        {visibleColumns.phone && <td>{elem.phone}</td>}
                        {visibleColumns.firmId && (
                          <td>
                            {
                              elem.firmId?.find(
                                (e) => e?._id === adminStorage.DX_AD_FIRM
                              )?.name
                            }
                          </td>
                        )}
