...
import { getCustomerReceiveList } from '../../../middleware/companyUser/comSerReport/comSerReport';
...
const CustomerReceiveList = () => {
  ...
  const { customerReceiveList, customerReceiveListLoading } = useSelector((state) => state?.comSerReport);
  ...
  useEffect(() => {
    if (customerReceiveList) {
      setServices(customerReceiveList?.services || []);
      setTotalItems(customerReceiveList?.pagination?.totalItems || 0);
    }
  }, [customerReceiveList]);
  ...
  const getPendingCompanyServices = () => {
    ...
    dispatch(getCustomerReceiveList(payload))
  }
  ...
  return (
    <div className="row">
      <div className="col-md-12">
        <PageHeader
          name={"Pending Customer Receive Services"}
          count={totalItems}
        ...
        <div className="card shadow-sm">
          ...
          {customerReceiveListLoading ? <LoadingSpinner text='' size='md' />
            : (
                      ...
                      {services.length > 0 ? (
                        services.map((service, index) => (
                          <tr key={service._id}>
                            ...
                            {visibleColumns.daysSinceDispatch && (
                              <td>
                                <span className={`badge ${service.daysSinceDispatch > 30 ? 'bg-danger' : service.daysSinceDispatch > 15 ? 'bg-warning' : 'bg-success'}`}>
                                  {service.daysSinceDispatch} day{service.daysSinceDispatch !== 1 ? 's' : ''}
                                </span>
                              </td>
                            )}
                            {visibleColumns?.companyDispatch && <td><DateTime value={service?.companyDispatch?.time} format='dateTime' /></td>}
