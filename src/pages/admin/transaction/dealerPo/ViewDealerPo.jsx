...
const ViewDealerPo = () => {
  ...
  const { id } = useParams();
  const [proObj, setProObj] = useState(null); // Changed to null for better null checks
  ...
  const getPoData = async () => {
    try {
      ...
      const url = `/admin/get-dealer-po/${id}`;
  ...
  const handleDownload = async () => {
    try {
      setDownload(true);
      const response = await get(`/admin/download-dealer-po/${id}`);
    ...
    return (
      <div className="row">
        <div className="col-md-12">
          <BreadCrumbs
            crumbs={[
              { label: "Dealer PO List", to: ADMIN_URLS.DEALER_PO_LIST },
          ...
          <div className="alert alert-danger">
            ...
            <div className="mt-3">
              <button
                ...
                onClick={() => navigate(ADMIN_URLS.DEALER_PO_LIST)}
              >
                Back to List
              </button>
            </div>
