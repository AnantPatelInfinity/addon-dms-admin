...
const InstallationPreviewCard = forwardRef(
  ({ formData, isPreview = true, firm, company }, ref) => {
      ...
      >
        ...
        <div className="d-flex justify-content-between align-items-center px-4 py-3 ">
          ...
          >
            <img
              src={
                adminStorage.DX_AD_FIRM_SN === "DMP"
                  ? logos.LOGIN_LOGO
                  : logos.X_TECH_LOGO
              }
              alt="Logo"
              style={{
                width: "100%",
                height: adminStorage.DX_AD_FIRM_SN === "DMP" ? "55px" : "90px",
                objectFit: "cover",
              }}
            />
          </div>
        ...
        >
          <div>
          {firm?.mobileNo} • {firm?.email} 
          </div>
          <div className="mt-1">
            {firm?.address} {firm?.addressTwo}{firm?.adressThree},    
            {firm?.city}, {firm?.state} - {firm?.pincode}
          </div>
        </div>
      </div>
    );
