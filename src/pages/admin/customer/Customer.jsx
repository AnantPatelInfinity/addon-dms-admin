...
const Customer = () => {
  ...
  const getCustomerData = async () => {
    try {
      setLoading(true)
      const response = await get(`/admin/get-customer?firmId=${adminStorage.DX_AD_FIRM}`);
  ...
  const handleDelete = async (elem) => {
    ...
    if (confirmResult.isConfirmed) {
      try {
        const response = await deleteById(`/admin/delete-customer`, elem._id);
        if (response.success) {
  ...
  const handleStatusUpdate = async (elem, status) => {
    ...
    if (confirmResult.isConfirmed) {
      try {
        const response = await post(`/admin/update-customer-status/${elem?._id}?status=${status}`);
        if (response.success) {
    ...
    <>
      <div className="row">
        <div className="col-md-12">
          <PageHeader name={"Customer List"} count={entity?.length} handleRefresh={handleRefresh} />
          <div className="card">
              ...
              <div className="row align-items-center">
                ...
                <div className="col-sm-8">
                  <div className="d-flex align-items-center flex-wrap row-gap-2 justify-content-sm-end">
                    ...
                    <Link to={ADMIN_URLS.MANAGE_CUSTOMER} className="btn btn-primary">
                        ...
                        <tr key={elem?._id}>
                          ...
                          <td style={{ textAlign: "end" }}>
                            <div className="dropdown table-action">
                              ...
                              >
                                <Link className="dropdown-item" to={ADMIN_URLS.VIEW_CUSTOMER} state={elem}>
                                  <i className="ti ti-eye text-blue" /> View
                                </Link>
                                <Link className="dropdown-item" to={ADMIN_URLS.MANAGE_CUSTOMER} state={elem}>
