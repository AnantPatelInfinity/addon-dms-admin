...
import DeRegiSection from './components/DeRegiSection';
import DeRegiBusinessSec from './components/DeRegiBusinessSec';

const DealerRegister = () => {
  const initialFormData = {
    firmId: '', dealerCompanyName: '', email: '', name: '', password: '', confirmPassword: '',
    address: '', addressTwo: '', addressThree: '', city: '', state: '', pincode: '', phone: '',
    gstNo: '', drugLicenseOne: '', drugLicenseTwo: '', panNo: '', image: null, previewImage: '',
    signature: null, previewSignature: '', agreeTerms: false, signatureFileType: ""
  };
  ...
  const handleFileChange = async (e, fieldName, allowPdf = false) => {
    ...
    try {
      ...
      if (response.data.success) {
        const fileUrl = response?.data?.data?.image || response?.data?.data?.pdf;
        const previewKey = `preview${fieldName?.charAt(0).toUpperCase() + fieldName?.slice(1)}`;
        const previewUrl = validImageTypes.includes(file.type) ? URL.createObjectURL(file) : null;

        setFormData(prev => ({
          ...prev,
          [fieldName]: fileUrl,
          [previewKey]: previewUrl,
          [`${fieldName}FileType`]: file.type // Store file type for later reference
        }));
      }
    } catch (error) {
      console.log(error, '!!');
  ...
  const validate = () => {
    ...
    const requiredFields = {
      firmId: 'Firm', dealerCompanyName: "Dealer Company Name", email: 'Email',
      name: 'Full name', password: 'Password', address: 'Address',
      city: 'City', state: 'State', pincode: 'Pincode', phone: 'Phone',
      gstNo: 'GST number', drugLicenseOne: 'Drug license', panNo: 'PAN number'
    };

    Object.entries(requiredFields).forEach(([field, name]) => {
      if (!formData[field]?.toString().trim()) newErrors[field] = `${name} is required`;
    });
  ...
  const handleSubmit = async (e) => {
    ...
    if (validate()) {
      ...
      Object.entries(formData).forEach(([key, value]) => {
        if (key !== 'previewImage' && key !== 'confirmPassword' && key !== "previewSignature") {
          submitData.append(key, value);
        }
      });

      try {
        const response = await axios.post(`${DX_URL}/admin/dealer-register`, submitData, {
          headers: {
    ...
    if (formData[previewKey]) {
      return (
        <div className="image-preview">
          <img src={formData[previewKey]} alt="Preview" className="preview-image" />
          <button type="button" className="change-image-btn" onClick={() => document.getElementById(fieldName).click()}>
            Change Image
          </button>
        </div>
      );
      ...
      return (
        <div className="upload-area" onClick={() => document.getElementById(fieldName).click()}>
  ...
  const renderInput = (name, label, type = 'text', options = {}) => (
    <div className={`form-group ${options.halfWidth ? 'half-width' : ''}`}>
      <label htmlFor={name}>{label} {options.required && <span className="required">*</span>}</label>
      {type === 'select' ? (
        <select
          id={name}
          name={name}
    ...
    <div className="password-strength-container mt-2">
      <div className="password-strength-bars">
        {[1, 2, 3, 4].map((level) => (
          <div
            ...
            style={{
              backgroundColor: checkPasswordStrength(formData.password) >= level
      ...
      {formData.password && (
        <div
          ...
          style={{ color: getPasswordStrengthColor(checkPasswordStrength(formData.password)) }}
        >
    ...
    <div className="dealer-register-container">
      ...
      <div className="register-card">
        <div className="register-form">
          ...
          <p className="welcome-text">Create your dealer account to get started</p>
          ...
          <form onSubmit={handleSubmit} encType="multipart/form-data">
            <div className="form-columns">
              ...
              <div className="form-column">
                <DeRegiBusinessSec renderInput={renderInput} formData={formData} errors={errors} handleChange={handleChange} />
                <button type="submit" className={`submit-btn ${isSubmitting ? 'loading' : ''}`} disabled={isSubmitting}>
                  {isSubmitting ? <><span className="spinner"></span> Registering...</> : 'Create Account'}
                </button>
