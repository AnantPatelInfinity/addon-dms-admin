...
const ViewCompany = () => {
    ...
    const handleStatusUpdate = async (status) => {
        ...
        const confirmResult = await Swal.fire({
            ...
            text: `Are you sure you want to set ${company?.name} to ${actionLabel.toLowerCase()}?`,
            icon: 'warning',
        ...
        if (confirmResult.isConfirmed) {
            try {
                const response = await post(`/admin/update-company-status/${company?._id}?status=${status}`);
                if (response.success) {
                    Swal.fire('Updated!', `${company?.name} status updated to ${actionLabel}.`, 'success');
                    ...
                    <div className="card-footer">
                        <div className="d-flex justify-content-between align-items-center">
                            ...
                            <button className="btn btn-outline-secondary" onClick={() => navigate(-1)}>
                                <i className="fas fa-arrow-left me-2"></i> Back to company List
                            </button>
                        </div>
                    </div>
                </div>
