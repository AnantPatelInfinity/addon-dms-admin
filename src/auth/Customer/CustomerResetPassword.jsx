...
const CustomerResetPassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setDisable(true);
    try {
      const response = await post(`${CUSTOMER_URLS.RESET_PASSWORD}`, {
        email,
        password: password.newPass,
        showPassword: password.confirmPass
      });
        ...
        <div className="login-form">
          ...
          <form onSubmit={handleSubmit}>
            <div className="form-group">
              ...
              <div className="password-input-container">
                <div className="input-with-icon">
                  ...
                  >
                    <i
                      className={`icon ${
                        password.newShow ? "eye-off-icon" : "eye-icon"
                      }`}
                    ></i>
                  </button>
                </div>
              ...
              {error.newPass && (
                <span className="error-message">{error.newPass}</span>
              )}
              <div className="password-requirements">
                <p>Password must contain:</p>
                <ul>
                  <li className={password.newPass.length >= 6 ? "valid" : ""}>
                    At least 6 characters
                  </li>
                  <li
                    className={/^[A-Z]/.test(password.newPass) ? "valid" : ""}
                  >
                    Start with capital letter
                  </li>
                  <li className={/\d/.test(password.newPass) ? "valid" : ""}>
                    At least one number
                  </li>
                  <li
                    className={
            ...
            <div className="form-group">
              ...
              <div className="password-input-container">
                <div className="input-with-icon">
                  ...
                  >
                    <i
                      className={`icon ${
                        password.confirmShow ? "eye-off-icon" : "eye-icon"
                      }`}
                    ></i>
                  </button>
                </div>
              ...
              {error.confirmPass && (
                <span className="error-message">{error.confirmPass}</span>
              )}
            </div>
