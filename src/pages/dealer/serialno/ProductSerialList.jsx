...
import { getDePoProductReceive, getPoSerialItems, resetDePoProductReceive } from '../../../middleware/PoItems/PoItems';
...
const ProductSerialList = () => {
    ...
    const normalColumnsConfig = [
        ...
        { key: "dealerPoNo", label: "Dealer PO No" },
    ...
    } = useSelector((state) => state?.dealerPoItems);
    ...
    useEffect(() => {
        if (dePoProductReceiveMessage) {
            Swal.fire({
                icon: 'success',
                title: 'Success!',
                text: dePoProductReceiveMessage,
                confirmButtonColor: '#198754'
            }).then(() => {
                fetchSerialNo()
            });
            dispatch(resetDePoProductReceive())
        }
        if (dePoProductReceiveError) {
            Swal.fire({
                icon: 'error',
                title: 'Error!',
                text: dePoProductReceiveError,
                confirmButtonColor: '#d33'
            });
            dispatch(resetDePoProductReceive())
        }
    }, [dePoProductReceiveMessage, dePoProductReceiveError])
    ...
    const handleProductReceive = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                dispatch(getDePoProductReceive(id));
            }
        });
    }
                                    ...
                                    <tbody className='text-center'>
                                        {commentsData?.map((elem, i) => (
                                            <tr key={elem?._id}>
                                                ...
                                                {viewMode === 'normal' && <td>{elem.dealerPoNo || elem.poNo || ""}</td>}
                                                ...
                                                {viewMode === 'normal' && visibleColumns.poAdminInvoice && (
                                                    <td className="">
                                                        <a
                                                            ...
                                                            style={{ cursor: elem.poAdminInvoice === null ? "not-allowed" : "pointer" }}
                                                            disabled={elem.poAdminInvoice === null}
                                                        >
                                                ...
                                                {viewMode === 'normal' && visibleColumns.poCompanyDispatch && (
                                                    <td className="">
                                                        <a
                                                            ...
                                                            style={{ cursor: elem.poCompanyDispatch === null ? "not-allowed" : "pointer" }}
                                                            disabled={elem.poCompanyDispatch === null}
                                                        >
                                                            <i className="ti ti-download me-1"></i>
                                                            <span>Dispatch</span>
                                                        </a>
