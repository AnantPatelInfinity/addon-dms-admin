...
const CustomerApproval = ({ serviceId, fetchServiceData, serviceObj }) => {
  ...
  const validateForm = () => {
    ...
    if (!form.dealerAmount) {
      newErrors.dealerAmount = "Dealer Amount is required";
    } else if (isNaN(form.dealerAmount)) {
      newErrors.dealerAmount = "Dealer Amount must be a number";
    } else if (parseFloat(form.dealerAmount) <= 0) {
      newErrors.dealerAmount = "Dealer Amount must be greater than 0";
    }

    if (!form.customerAmount) {
      newErrors.customerAmount = "Customer Amount is required";
    } else if (isNaN(form.customerAmount)) {
      newErrors.customerAmount = "Customer Amount must be a number";
    } else if (parseFloat(form.amount) <= 0) {
      newErrors.customerAmount = "Customer Amount must be greater than 0";
    }
  ...
  const getApiUrl = () => {
    return serviceObj?.isParts === true
      ? `/admin/admin-customer-part-approval/${serviceId}`
      : `/admin/admin-customer-approval/${serviceId}`;
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.error(
        error?.response?.data?.message || "Error submitting approval"
      );
    } finally {
      setDisable(false);
    }
  ...
  // const isPaymentStatus2 =
  //   Number(serviceObj?.customerApproval?.paymentStatus) === 2;
