...
import { resetCuPoDownload, getCustomerPoDownload, getCustomerPoList } from "../../../middleware/customerUser/customerPo/customerPo";
...
const PoList = () => {
  ...
  const fetchData = () => {
    ...
    dispatch(getCustomerPoList(formData));
  };
  ...
  }, [currentPage, search, limit, cuPoList]);
  ...
  return (
    <div className="row">
      <div className="col-md-12">
        <PageHeader
          name={"PO List"}
          count={getCustomerPoList?.length}
        ...
        {cuPoListError && (
          <div className="alert alert-danger text-center" role="alert">
            {cuPoListError || "Something went wrong while fetching data."}
          </div>
        )}
        {!cuPoListLoading && !cuPoListError && (
          <div className="card">
                    ...
                    {commentsData?.map((elem, i) => (
                      <tr key={elem?._id}>
                        ...
                        {visibleColumns["voucherNo"] && (
                          <td>{elem?.voucherNo || "-"}</td>
                        )}
                        {visibleColumns["poDate"] && (
                          <td>
                            {elem?.poDate
                              ? moment(elem?.poDate).format("DD-MM-YYYY")
                              : "-"}
                          </td>
                        )}
                        {/* {visibleColumns["dealerName"] && (
                          <td>{elem?.dealerName || "-"}</td>
                        )} */}
                        ...
                        {visibleColumns["shipTo.state"] && (
                          <td>{elem?.shipTo?.state || "-"}</td>
                        )}
                        {visibleColumns["totalQuantity"] && (
                          <td>{elem?.totalQuantity || "-"}</td>
                        )}
                        {visibleColumns["totalAmount"] && (
                          <td>{formatAmount(elem?.totalAmount)}</td>
                        )}
