...
const CustomerApproval = ({ serviceId, fetchServiceData, serviceObj }) => {
  ...
  const validateForm = () => {
    ...
    if (!form.dealerAmount) {
      newErrors.dealerAmount = "Amount is required";
    } else if (isNaN(form.dealerAmount)) {
      newErrors.amount = "Amount must be a number";
    } else if (parseFloat(form.dealerAmount) <= 0) {
      newErrors.dealerAmount = "Amount must be greater than 0";
    }
  ...
  const getApiUrl = () => {
    return serviceObj?.isParts === true
      ? `${DX_URL}/dealer/dealer-customer-part-approval/${serviceId}`
      : `${DX_URL}/dealer/dealer-customer-approval/${serviceId}`;
  };
