...
import {
  ...
  getAllInstallation,
  resetDownloadInstallation,
} from "../../../middleware/customerUser/customerInstallation/installation";
...
const InstallationList = () => {
  ...
  } = useSelector((state) => state?.customerInstallation);

  useEffect(() => {
    if (downloadInstallationMessage && downloadInstallation && downloadInstallation?.pdfUrl) {
  ...
  }, [downloadInstallationMessage, downloadInstallation, downloadInstallationError]);
  ...
  const fetchData = () => {
    ...
    formData.append("isOldData", "false");
  ...
  const handleDownload = (id, dealerId) => {
    const isDealer = dealerId ? true : false;
    const payload = { isDealer }
    dispatch(downloadInstallationReport(payload, id));
  };
              ...
              <div className="row align-items-center">
                ...
                {/* <div className="col-sm-8">
                  <div className="d-flex align-items-center flex-wrap row-gap-2 justify-content-sm-end">
                    ...
                    <Link
                      to={CUSTOMER_URLS.MANAGE_INSTALL}
                  ...
                  <tbody>
                    {commentsData?.map((elem, i) => (
                      <tr key={elem?._id}>
                        ...
                        {visibleColumns["reportNo"] && (
                          <td> {elem?.reportNo}</td>
                        )}
                        {visibleColumns["customerFirstName"] && (
                          <td>
                            {elem?.customerTitle} {elem?.customerFirstName}{" "}
                            {elem?.customerLastName}
                          </td>
                        )}
                        {visibleColumns["productSerialNo"] && (
                          <td>{elem?.productSerialNo}</td>
                        )}
                        {visibleColumns["registerDate"] && (
                          <td>
                            <DateTime
                              value={elem?.registerDate}
                              format="date"
                            />
                          </td>
                        )}
                        {visibleColumns["totalInstallDay"] && (
                          <td>
                            {formatInstallationDays(elem?.physicalInstallDate)}
                          </td>
                        )}
                        {visibleColumns["rejectReason"] && (
                          <td>{elem?.rejectReason || "-"}</td>
                        )}
                          ...
                          <div className="dropdown table-action">
                            ...
                            <div className="dropdown-menu dropdown-menu-right">
                              <Link
                                ...
                                to={CUSTOMER_URLS.VIEW_INSTALLATION}
                                state={elem?._id}
                              >
