...
const ComOtpVerify = () => {
  ...
  useEffect(() => {
    const storedExpiry = localStorage.getItem('com_otpExpiry');
    const now = Date.now();
    ...
    if (storedExpiry) {
      ...
      localStorage.setItem('com_otpExpiry', expiry.toString());
  ...
  const handleVerify = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await axios.post(`${DX_URL}/company/verify-otp`, { email, otp });
      localStorage.removeItem('com_otpExpiry');
      const { success, message } = res?.data;
      if (success) {
        toast.success(message);
        navigate(COMPANY_URLS.RESET_PASSWORD, { state: email });
      } else {
        toast.error(message);
      }
    } catch (err) {
  ...
  const handleResendOtp = async () => {
    try {
      ...
      const res = await axios.post(`${DX_URL}/company/forgot-password`, { email });
      const { success, message } = res;
      if (success) {
        toast.success(message);
        setResendEnabled(false);
        const newExpiry = Date.now() + 120000; // Changed to 2 minutes (120000 ms)
        localStorage.setItem('com_otpExpiry', newExpiry.toString());
        setTimeLeft(120);  // Changed to 2 minutes (120 seconds)
